...
```console
opam switch create cs3110-2026fa ocaml-base-compiler.5.3.0
```
...
```
#  switch         compiler
→  cs3110-2026fa  ocaml-base-compiler.5.3.0,ocaml-options-vanilla.1
```
...
- **Is your switch listed?** Run `opam switch list` and make sure a switch named
  `cs3110-2026fa` is listed, that it has the 5.3.0 compiler, and that it is the
  active switch (which is indicated with an arrow beside it). If that switch is
  present but not active, run `opam switch cs3110-2026fa` then see whether utop
